...
from austin.analysers import IssueAnalyser
from austin.github import GithubApi


class TestIssueAnalyserClassAndSimpleMethods(unittest.TestCase):
    def setUp(self):
        self.analyser = IssueAnalyser('octocat', 'Hello-World', '2020-01-01', '2020-02-02')
    ...
    def test_days_to_become_stale(self):
        self.assertEqual(self.analyser.DAYS_TO_BECOME_STALE, 14)

    def test_base_search_parameters(self):
        self.assertEqual(
            self.analyser.base_search_parameters,
            {'repo': 'octocat/Hello-World', 'type': 'issue'}
        )
